...
public:
	...
	Terreno(HWND hWnd, OpenGLClass* OpenGL, const wchar_t* alturas,
		const wchar_t* textura, const wchar_t* textura2,
		const wchar_t* textura3, const wchar_t* textura4,
		const wchar_t* textura5, const wchar_t* textura6,
		const wchar_t* Blend,
		float ancho, float prof,
		int numtext, int numtext2,
		int numtext3, int numtext4,
		int numtext5, int numtext6,
		int blendId)
	{
		anchof = ancho;
		proff = prof;
		...
		terreno = Plano(Ancho(), Alto(), ancho, prof, Dir_Imagen(), 40); //Ese numero es tiling que es que tanto se repite la textura
		deltax = anchof / Ancho();                                  //A mas numero mas sensacion de tamños
		deltaz = proff / Alto();
		verx = Ancho();
		verz = Alto();
		...
		EnviaTextura(hWnd, OpenGL, textura, numtext);
		...
		EnviaTextura(hWnd, OpenGL, Blend, blendId);
		
	}

	void EnviaTextura(HWND hwnd, OpenGLClass* openGL, const wchar_t* textura, int numtext)
	{
